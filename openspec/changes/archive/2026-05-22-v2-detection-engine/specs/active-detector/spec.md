...
The `ActiveDetector` SHALL implement directional validation to prevent false positives caused by "quiet day" patterns (negative deviations dominating the score).

#### Scenario: Direction reversal logic
- **WHEN** score > threshold
- **THEN** SHALL compute eUp (positive deviation energy): `Σ w_i × z_i²` for dimensions where z_i > 0
- **AND** eDown (negative deviation energy): `Σ w_i × z_i²` for dimensions where z_i < 0
- **AND** ratio = `eDown / (eUp + eDown + 1e-10)`
- **WHEN** ratio > directionThreshold (default 0.75)
- **THEN** the result SHALL be reversed to NORMAL
- **AND** `directionReversed` SHALL be `true`

#### Scenario: Direction validation disabled
- **WHEN** `directionThreshold = 0`
- **THEN** no reversal SHALL occur regardless of ratio
...
- **AND** all arrays SHALL be length 14

### Requirement: ActiveDetector output
The `ActiveDetector` SHALL produce a `DetectionResult` with complete dimension analysis.

#### Scenario: Detection result includes dimension reports
- **WHEN** a detection is complete
- **THEN** the result SHALL include 14 `DimensionReport` entries, each with value, zScore, contribution, weight, description
- **AND** `topDeviations` SHALL list dimensions sorted by |zScore| descending (top 5)
- **AND** direction validation info SHALL be included (ratio, eUp, eDown, reversed)
