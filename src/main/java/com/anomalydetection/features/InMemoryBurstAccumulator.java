...
public class InMemoryBurstAccumulator implements Closeable {
    private final List<OpEntry> entries;
    private boolean closed = false;

    public static InMemoryBurstAccumulator create() {
        return new InMemoryBurstAccumulator();
    }

    private InMemoryBurstAccumulator() {
        this.entries = new ArrayList<>();
    }

    public void write(long epochSeconds, String opType) throws IOException {
        if (closed) {
            throw new IOException("InMemoryBurstAccumulator is closed");
        }
        entries.add(new OpEntry(epochSeconds, opType));
    }

    public List<OpEntry> sortAndRead() {
        List<OpEntry> sorted = new ArrayList<>(entries);
        sorted.sort(Comparator.comparingLong(OpEntry::epochSeconds));
        return sorted;
    }

    public BurstFeatures computeBurstFeatures() {
